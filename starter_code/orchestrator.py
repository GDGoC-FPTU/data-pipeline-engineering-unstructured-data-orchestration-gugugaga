    ...
    for file_path in pdf_files:
        ...
        if run_semantic_checks(processed):
            try:
                ...
                final_kb.append(doc.model_dump() if hasattr(doc, 'model_dump') else doc.dict())
            except Exception as e:
                print(f"Validation failed for PDF {file_path}: {e}")
    ...
    for file_path in video_files:
        ...
        if run_semantic_checks(processed):
            try:
                ...
                print(f"Validation failed for Video {file_path}: {e}")
